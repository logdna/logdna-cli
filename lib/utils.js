...
function apiCall(config, endpoint, method, params, callback) {
    ...
    got((config.LOGDNA_APISSL ? 'https://' : 'http://') + config.LOGDNA_APIHOST + '/' + endpoint, opts)
        ...
        .catch(err => {
            ...
            } else return log('Error ' + err.statusCode + ': ' + err.response.body);
        });
};

function levelColor(config, level) {
    if ((new RegExp(config.ERROR_LEVEL_TEST)).test(level)) return '\x1b[38;5;255;48;5;203m';
    if ((new RegExp(config.WARN_LEVEL_TEST)).test(level)) return '\x1b[38;5;255;48;5;208m';
    ...
    if (force || Date.now() - (config.updatecheck || 0) > config.UPDATE_CHECK_INTERVAL) {
        ...
        got(config.VERSION_CHECK_URL, { timeout: (force ? force_timeout_ms : default_timeout_ms) })
            .then(res => {
                ...
                } else return callback && callback(); // no update necessary, run rest of program
            })
            .catch(function(err) {
                return log('Error ' + err.statusCode + ': ' + err.response.body);
            });

    } else return callback && callback(); // no check necessary, run rest of program
};
