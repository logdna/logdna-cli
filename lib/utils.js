...
const renderLine = function(config, line, params) {
    ...
    if (!line._host || !line._app) return;
    ...
    if (config.SUPPORTS_COLORS) {
        return log('\x1b[38;5;240m' + t.toString().substring(4, 11) + t.toString().substring(16, 24) +
    ...
    return log(t.toString().substring(4, 11) + t.toString().substring(16, 24) +
        ' ' + line._host +
        ' ' + line._app +
