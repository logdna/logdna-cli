...
// Convenience function to create standardized developer log object
const devLogPrim = function( {message, who, what, why, where, when, additional } ) {
    // Message to be displayed in LogDNA LiveTail. Additional fields/objects will be stored, searchable and actionable
    if (typeof message == 'undefined') message = "";

    // Who is sending this log.  Often user but here it's dev
    if (typeof who == 'undefined') who = "dev";

    //  High level "what" is happening
    if (typeof what == 'undefined') what = "";

    // Why the log is occuring
    if (typeof why == 'undefined') why = "";

    // Where in your system this is happening
    if (typeof where == 'undefined') where = "local";

    // When this occured in your system.  Can be different then the time of the log itself
    if (typeof when == 'undefined') when = new Date().toISOString();

    // Additional fields to include in the log.  An object is required 
    if (typeof additional != 'undefined' && typeof additional != 'object') {
        console.error('If additional parameter is defined, it must be an object')
    }
