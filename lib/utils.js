...
function apiCall(config, endpoint, method, params, callback) {
    ...
    request(opts, (error, response, body) => {
        if (error) {
            if (response && response.statusCode.toString() === '403') return callback('Access token invalid. If you created or changed your password recently, please \'logdna login\' again. Type \'logdna --help\' for more info.');
    ...
    if (force || Date.now() - (config.updatecheck || 0) > config.UPDATE_CHECK_INTERVAL) {
        ...
        request.get(config.VERSION_CHECK_URL, {
            ...
            if (!semver.valid(body)) {
                ...
                console.log(`performUpgrade v1: ${JSON.stringify(config).length}`);
            ...
            console.log(`performUpgrade v2: ${JSON.stringify(config).length}`);
