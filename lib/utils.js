...
function apiCall(config, endpoint, method, params, callback) {
    ...
    if (!params) params = {};
    if (params.auth === false) { // no auth (login, register)
        delete params.auth;
    } else if (typeof params.auth === 'string') { // using HTTP Auth
        if (params.auth) {
        ...
        delete params.auth;
    } else { // Using HMAC token stored in config
    ...
    request(opts, (error, response, _body) => {
        var body = null;
        try {
        ...
        if (error || body.error) {
            ...
            console.log(body);
            return callback('Error: ' +  error ? error : body.error);
        }
